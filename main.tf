...
resource "null_resource" "get_cluster_credentials" {
  ...
  provisioner "local-exec" {
    command = "sleep 60 && gcloud config set project gke-terraform && gcloud config set compute/zone europe-west3-a && gcloud container clusters get-credentials ${var.cluster_name}"
  }
}
